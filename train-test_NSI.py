# -*- coding: utf-8 -*-
"""
Created on Thu Aug  9 12:28:09 2018
...
def get_sent_img_feats(config, data, num_words, embedding_matrix):
    ...
    [encoded_sents, encoded_imgs] = SentEncoder.predict(data,
                                                        batch_size=128)
    num_sents = 5
    feat_temp = []
    labels_temp = []
...
def train(config, train_data, num_words, embedding_matrix):
    ...
    if 'DATALOADER' in config:
        if 'trainImgId' in config['DATALOADER']:
            trainimgid = config['DATALOADER']['trainImgId']
    if 'MODEL_Story_ImgSeq_PARAMS' in config:
        if 'epochs' in config['MODEL_Story_ImgSeq_PARAMS']:
            epochs = config['MODEL_Story_ImgSeq_PARAMS'].getint('epochs')
        if 'batchsize' in config['MODEL_Story_ImgSeq_PARAMS']:
            BS = config['MODEL_Story_ImgSeq_PARAMS'].getint('batchsize')
    if 'OUTPUTFILENAMES' in config:
        if 'NSImodelname' in config['NSImodelname']:
            NSImodelname = config['OUTPUTFILENAMES']['NSImodelname']

    try:
        train_image
    except NameError:
        train_image = csvnumread(trainimgid)

    # Load Story Encoder model architecture
    StoryEncoder = modelArch.Model_Story_ImgSeq_NSI()
    ...
    for i in range(0, epochs):
        ...
        StoryEncoder.fit(x_train1, y_train1, epochs=1, batch_size=BS,
                         verbose=1)
        print("epoch: ", i, " completed")

    StoryEncoder.save(NSImodelname)


def test(config, test_data, num_words, embedding_matrix):

    if 'DATALOADER' in config:
        if 'testSamplesFileName' in config['DATALOADER']:
            testinsamplename = config['DATALOADER']['testSamplesFileName']
    if 'OUTPUTFILENAMES' in config:
        if 'NSImodelname' in config['OUTPUTFILENAMES']:
            NSImodelname = config['OUTPUTFILENAMES']['NSImodelname']
        if 'testImgOutFeatSaveName' in config['OUTPUTFILENAMES']:
            testoutfeatname = config['OUTPUTFILENAMES'][
                    'testImgOutFeatSaveName']
    ...
    out_fea = model_lstm.predict(sent_test200)
...
if __name__ == '__main__':
    ...
    if 'DEFAULT' in config:
        if 'booltrain' in config['DEFAULT']:
            booltrain = config['DEFAULT'].getboolean('booltrain')
        if 'booltest' in config['DEFAULT']:
            booltest = config['DEFAULT'].getboolean('booltest')
